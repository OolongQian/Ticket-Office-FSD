...
bool sjtu::processor::task_query_profile()  {
    ...
    if (val.null_obj()) {
        ...
        printf("%s %s %s %s %s %hd\n", key.username, val.name, val.password, val.email, val.phone, val.privilege);
        return true;
    }
}
...
bool sjtu::processor::task_admin_query_train(sjtu::train_info_val &ret, bool &sold)  {
    char id[ID_SIZE];
    train_id_val val;
...
bool sjtu::processor::task_buy_ticket()  {
    ...
    if(!get_train_info(train_id, train_info)) {
        ...
        /// note raw input date is already the start date of head station.
    ...
    for(int l = 0; l < cat_len; ++l) {
        ...
        for (int i = 0; i < len2; ++i) {
            ...
            for (char j = 0; j < station_idx; ++j) {
                ...
                char date_head[11];
                strcpy(date_head, date);
                modify_date(date_head, - train_info2.stations[station_idx1].start_date_offset);
...
void sjtu::processor::print_bill(const sjtu::bill_key &b_key, const sjtu::bill_val &b_val) {
    ...
    // arrive date
    start_date_offset = train_info.stations[b_key.station_idx2].start_date_offset - train_info.stations[b_key.station_idx1].start_date_offset;
    date_to_str(b_key.date + start_date_offset, date_str);
    printf("%s ", date_str);
    // arrive time
...
void sjtu::processor::fill_query_ticket(const train_info_val &train_info, const int &station_idx1, const int &station_idx2,
    ...
    modify_date(date, train_info.stations[station_idx1].start_date_offset);
    ...
    /// note! we need to change date. we need to change it to be arrive date.
    short arrive_date_offset = train_info.stations[station_idx2].start_date_offset;
    if(train_info.stations[station_idx2].start > train_info.stations[station_idx2].arrive)
        ++arrive_date_offset;
    strcpy(date, date_head);
    modify_date(date, arrive_date_offset);
...
void sjtu::processor::master()  {
    while (true) {
        ...
        else if (cmd == ADMIN_QUERY_TRAIN) {
            train_info_val ret;
            bool sold;
            if (task_admin_query_train(ret, sold)) {
                printf("%d\n", 1);
                if(sold)
                    printf("%d\n", 1);
                else
...
void sjtu::processor::init_open_file() {
    user_db->open_file();
...
void sjtu::processor::close_file() {
    user_db->close_file();
