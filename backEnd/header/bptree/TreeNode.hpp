...
template<class KeyType, class ValType>
short TreeNode<KeyType, ValType>::search_sup(KeyType K) {
    short pos = 0;
    if (keys.size() < 10) {
...
template<class KeyType, class ValType>
short TreeNode<KeyType, ValType>::search_exact(const KeyType &K) {
    if (keys.size() < 10) {
        short pos = 0;
        while (pos < keys.size() && keys[pos] != K) ++pos;
...
template<class KeyType, class ValType>
short TreeNode<KeyType, ValType>::search_child(offsetNumber child)  {
    if (childs.size() < 10) {
        short i = 0;
